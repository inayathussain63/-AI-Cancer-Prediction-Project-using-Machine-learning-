...
def load_data(filepath='d:/x pro/srint 2/final_cleaned_data.csv'):
    ...
    # Always generate structured data for better model performance
...
def generate_dummy_data(n_samples=1000):
    """Generate structured synthetic data with realistic health risk patterns"""
    np.random.seed(42)
    ...
    data = {
        ...
        'Alcohol_Consumption': np.random.choice(['High', 'Moderate', 'Low'], n_samples, p=[0.2, 0.3, 0.5]),
        'Diabetes': np.random.choice(['Yes', 'No'], n_samples, p=[0.15, 0.85]),
        'Hypertension': np.random.choice(['Yes', 'No'], n_samples, p=[0.25, 0.75]),
    ...
    # Calculate Risk_Score based on multiple factors with non-linear relationships
    risk_score = np.zeros(n_samples)
    
    # Age factor (non-linear: risk increases exponentially after 50)
    risk_score += np.where(df['Age'] < 50, df['Age'] * 0.3, 
                           15 + (df['Age'] - 50) * 1.2)
    
    # Smoking (major risk factor)
    risk_score += np.where(df['Smoking'] == 'Yes', 25, 0)
    
    # Alcohol (moderate effect)
    alcohol_map = {'High': 15, 'Moderate': 5, 'Low': 0}
    risk_score += df['Alcohol_Consumption'].map(alcohol_map)
    
    # Chronic conditions (high impact)
    risk_score += np.where(df['Diabetes'] == 'Yes', 20, 0)
    risk_score += np.where(df['Hypertension'] == 'Yes', 15, 0)
    risk_score += np.where(df['Heart_Disease'] == 'Yes', 30, 0)
    
    # BMI (non-linear: both underweight and overweight increase risk)
    bmi_risk = np.where(df['BMI'] < 18.5, (18.5 - df['BMI']) * 2,
                        np.where(df['BMI'] > 30, (df['BMI'] - 30) * 1.5, 0))
    risk_score += bmi_risk
    
    # Blood pressure (high BP increases risk)
    risk_score += np.where(df['Systolic_BP'] > 140, (df['Systolic_BP'] - 140) * 0.3, 0)
    risk_score += np.where(df['Diastolic_BP'] > 90, (df['Diastolic_BP'] - 90) * 0.4, 0)
    
    # Blood sugar (diabetes indicator)
    risk_score += np.where(df['Blood_Sugar'] > 126, (df['Blood_Sugar'] - 126) * 0.2, 0)
    
    # Cholesterol
    risk_score += np.where(df['Cholesterol'] > 240, (df['Cholesterol'] - 240) * 0.15, 0)
    
    # Exercise (protective factor - reduces risk)
    risk_score -= df['Exercise_Hours_Week'] * 2
    
    # Hospital visits (indicator of health issues)
    risk_score += df['Hospital_Visits_Year'] * 3
    
    # Add some random noise (10% variance)
...
def optimize_model(X_train, y_train):
    ...
    for name, config in models_params.items():
        ...
        # Enhanced tuning for Random Forest
        if name == 'RandomForest':
            print("  -> Applying intensive search for Random Forest...")
            n_iter_search = 50
            cv_folds = 5
        else:
            n_iter_search = 10
...
def main():
    ...
    # Perform SHAP analysis (disabled for faster execution)
